...
lessThanSuc : ∀ ( a  b : ℕ ) → a ≤  b ≡ tt → a < suc ( b ) ≡ tt
lessThanSuc zero b x = refl
lessThanSuc (suc a) b x  = {!lessThanSuc a b x !}
...
length-filter' (x :: l) p | ff rewrite length-filter' l p = {!length-filter' l p!}
